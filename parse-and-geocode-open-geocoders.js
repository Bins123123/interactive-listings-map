...
// This standalone entry point always uses Geocodio for U.S. listings and
// public Nominatim for the small international remainder.
process.env.GEOCODIO_API_KEY ||= process.env.GEOCODIO_API;
process.env.NOMINATIM_USER_AGENT ||= "BinswangerListingsMap/1.0 (nicolebachman12@gmail.com)";
...
const US_STATE_CODES = new Set([
  "AL", "AK", "AS", "AZ", "AR", "CA", "CO", "CT", "DC", "DE", "FL", "GA",
...
async function geocodeWithGeocodio(query, geocodioApiKey) {
  ...
  if (!response.ok) {
    throw new Error(`Geocodio returned HTTP ${response.status}`);
  }
...
async function geocodeWithNominatim(query, userAgent) {
  ...
  const payload = await response.json();
  const result = payload && payload[0];
  const longitude = Number(result && result.lon);
  const latitude = Number(result && result.lat);
...
async function main() {
  ...
  // GEOCODIO_API is accepted for compatibility with an existing GitHub secret.
  const geocodioApiKey = process.env.GEOCODIO_API_KEY || process.env.GEOCODIO_API;
  const nominatimUserAgent = process.env.NOMINATIM_USER_AGENT;
  if (!geocodioApiKey || !nominatimUserAgent) {
    console.error(
      "GEOCODIO_API_KEY (or GEOCODIO_API) and NOMINATIM_USER_AGENT are required for the Geocodio/Nominatim version."
    );
    process.exitCode = 1;
    return;
  }
