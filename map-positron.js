    ...
    function emphasizeStateBoundaries() {
      const styleLayers = map.getStyle()?.layers || [];
      ...
      for (const layer of styleLayers) {
        ...
        const isAdministrativeBoundary =
          layer.type === "line" &&
          (/boundary|admin|state/i.test(layerId) || /boundary/i.test(sourceLayer));
        ...
        map.setPaintProperty(layerId, "line-color", "#c4c7c2");
        map.setPaintProperty(layerId, "line-opacity", 0.5);
        map.setPaintProperty(layerId, "line-width", 0.7);
      }
    }

    function addStateBoundaryOverlay() {
      ...
      map.addLayer({
        ...
        // OSM administrative level 4 represents U.S. states and equivalent regions.
        filter: ["any", ["==", "admin_level", 4], ["==", "admin_level", "4"]],
        minzoom: 0,
        paint: {
          "line-color": ["interpolate", ["linear"], ["zoom"], 0, "#d0d2ce", 10, "#c3c6c1", 14, "#aeb3ad", 18, "#989e97"],
          "line-width": ["interpolate", ["linear"], ["zoom"], 0, 0.45, 6, 0.8, 10, 1.05, 14, 1.45, 18, 1.8],
          "line-opacity": ["interpolate", ["linear"], ["zoom"], 0, 0.4, 10, 0.52, 14, 0.62, 18, 0.72],
          "line-dasharray": [2, 1]
        }
      });
    }

    function addDistantStateLabels() {
      if (map.getLayer("distant-state-labels")) {
        return;
      }

      map.addLayer({
        id: "distant-state-labels",
        type: "symbol",
        source: "state-boundaries",
        "source-layer": "place",
    ...
    function addClassicHighwayOverlay() {
      if (!map.getSource("classic-highways")) {
        map.addSource("classic-highways", {
          type: "vector",
          url: "https://tiles.openfreemap.org/planet/latest"
        });
      }
      ...
      if (!map.getLayer("classic-highway-outline")) {
        map.addLayer({
          id: "classic-highway-outline",
          type: "line",
          source: "classic-highways",
          ...
          paint: {
            "line-color": "#5e665f",
            "line-width": ["interpolate", ["linear"], ["zoom"], 8, 1.1, 10, 2.1, 14, 4.3, 18, 7.8],
            "line-opacity": 0.58
          }
        }, firstSymbolLayerId);
      }

      if (map.getLayer("classic-highways")) {
        return;
      }
      ...
      map.addLayer({
        ...
        // Keep the national overview quiet, then reveal a familiar street-map palette.
        minzoom: 8,
        paint: {
          "line-color": [
            "match",
    ...
    function addDetailedBuildingOverlay() {
      if (!map.getSource("detailed-buildings")) {
        map.addSource("detailed-buildings", {
          type: "vector",
          url: "https://tiles.openfreemap.org/planet/latest"
        });
      }

      if (map.getLayer("detailed-buildings")) {
        return;
      }
      ...
      map.addLayer({
        ...
        type: "fill",
        source: "detailed-buildings",
        "source-layer": "building",
        minzoom: 14,
        paint: {
          "fill-color": "#cdccc5",
          "fill-opacity": 0.7,
          "fill-outline-color": "#aeb0aa"
        }
      }, firstSymbolLayerId);
    }

    function revealHighwayShieldsAtOverlayZoom() {
      const overlayZoom = 8;
      const interstateLayerId = "highway-shield-us-interstate";
      const interstateLayer = map.getStyle()?.layers?.find(
        (layer) => layer.id === interstateLayerId
      );

      if (!interstateLayer || !map.getLayer(interstateLayerId)) {
        return;
      }

      // Keep U.S. and state-route shields at the style's closer zoom level.
      // Only Interstate shields share the colored-highway threshold.
      map.setLayerZoomRange(
        interstateLayerId,
        Math.min(interstateLayer.minzoom ?? overlayZoom, overlayZoom),
        interstateLayer.maxzoom ?? 24
      );
      map.setLayoutProperty(interstateLayerId, "symbol-spacing", [
    ...
    function warmLandPaletteSlightly() {
      ...
      for (const layer of styleLayers) {
        const layerId = String(layer.id || "");
        if (layer.type === "background" && /^(background|land)$/i.test(layerId)) {
          map.setPaintProperty(layerId, "background-color", "#f1f1ee");
        }

        if (layer.type === "fill" && /landcover|landuse|park|wood|grass/i.test(layerId)) {
          map.setPaintProperty(
            layerId,
            "fill-color",
            /park|wood|grass/i.test(layerId) ? "#dbe1d8" : "#e8e8e4"
          );
        }

        if (layer.type === "line" && /land-structure/i.test(layerId)) {
          map.setPaintProperty(layerId, "line-color", "#dfe0da");
        }
      }
    }
    ...
    function fitToFeatures(features) {
      ...
      map.easeTo({
        center: [-98.5, 39],
        zoom: window.innerWidth <= 640 ? 2.1 : 2.6,
        duration: 0
      });
    }
    ...
    async function loadListings() {
      try {
        ...
        emphasizeStateBoundaries();
        addStateBoundaryOverlay();
        addDistantStateLabels();
        addDetailedBuildingOverlay();
        addClassicHighwayOverlay();
        revealHighwayShieldsAtOverlayZoom();
        ...
        function createPopupForPoint(point) {
          ...
          popup = new maplibregl.Popup(popupOptions);
          popup.on("close", () => {
            popup = null;
            syncMobilePopupState();
          });
